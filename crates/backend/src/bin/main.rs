...
use backend::{settings, Error, logger};
...
fn main() -> Result<(), Report<Error>>
{
    ...
    let configs_dir =
        settings::get_configs_dir_path( "./configs/backend/", &[&env_prefix, "_CONFIGS_DIR"].concat(), &cli_args.configs_dir ).into_report().change_context( Error::InvalidConfigsDir )?;
